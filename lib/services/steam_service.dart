...
class SteamService {
  ...
  /// Update a single game's compatibility tool in config.vdf.
  Future<bool> updateCompatTool(int appId, String? newCtool) async {
    return updateCompatTools({appId: newCtool});
  }

  /// Update multiple games' compatibility tools in config.vdf (batch operation).
  /// This is the core of ProtoGNOME's "apply to all" feature.
  Future<bool> updateCompatTools(Map<int, String?> gameToolMap) async {
    final cfgDir = getSteamConfigDir();
    if (cfgDir == null) return false;
    ...
    await configFile.copy('${configFile.path}.protognome_backup');
    ...
    try {
      ...
      // Try both 'Valve' and 'valve' case variants
      final valveKey = software.containsKey('Valve') ? 'Valve' : 'valve';
      final valve = software[valveKey] as Map<String, dynamic>?;
      if (valve == null) return false;
      final steam = valve['Steam'] as Map<String, dynamic>?;
      if (steam == null) return false;

      // Get or create CompatToolMapping
      final compatMapping =
          (steam['CompatToolMapping'] as Map<String, dynamic>?) ?? {};
      steam['CompatToolMapping'] = compatMapping;

      for (final entry in gameToolMap.entries) {
        final appIdStr = entry.key.toString();
        final newCtool = entry.value;

        if (newCtool == null) {
          compatMapping.remove(appIdStr);
        } else if (compatMapping.containsKey(appIdStr)) {
          (compatMapping[appIdStr] as Map<String, dynamic>)['name'] = newCtool;
        } else {
          compatMapping[appIdStr] = {
            'name': newCtool,
            'config': '',
            'priority': '250',
          };
      ...
      // Write back
      final newContent = VdfParser.dump(parsed);
      await configFile.writeAsString('"InstallConfigStore"\n{\n$newContent}\n');

      return true;
    } catch (e) {
      print('Error updating compat tools: $e');
      // Restore backup on failure
      try {
        final backup = File('${configFile.path}.protognome_backup');
        if (backup.existsSync()) {
          await backup.copy(configFile.path);
        }
      } catch (_) {}
      return false;
    }
  ...
  /// Apply a single Proton version to ALL installed games.
  /// This is the key ProtoGNOME-exclusive feature.
  Future<bool> applyCompatToolToAllGames(
      String compatToolName, List<SteamGame> games) async {
    final mapping = <int, String?>{};
    for (final game in games) {
      mapping[game.appId] = compatToolName;
    }
    return updateCompatTools(mapping);
  }

  /// Remove a compatibility tool from all games that use it.
  Future<bool> removeCompatToolFromAllGames(
      String compatToolName, List<SteamGame> games) async {
    final mapping = <int, String?>{};
    for (final game in games) {
      if (game.compatTool == compatToolName) {
        mapping[game.appId] = null;
      }
    }
